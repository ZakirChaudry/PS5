...
province_ranges %>% ggplot(aes(fill=Dataset, y=percent, x=cps19_province)) + geom_bar(position = "dodge", stat = "identity") + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))

```
For future reference, the supported party of each respondent, after our cleaning, is outlined below. 
```{r CES party}
ces %>% ggplot(aes(x=cps19_votechoice)) + geom_bar()
```
## Model
...
``` {r post-strat results, echo=FALSE}
gss %>% ggplot(aes(x = party)) + geom_bar()
...
party_ranges %>% ggplot(aes(fill=Dataset, y=percent, x=party)) + geom_bar(position = "dodge", stat = "identity") + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
...
results %>% ggplot(aes(x=party, y=votes)) + geom_bar(stat = "identity")
